...
from core.calculations import calcular_precio_final

def _preguntar_exportar_pdf() -> bool:
    """Pregunta al usuario si desea exportar el reporte a PDF."""
    print("\n" + "-" * 35)
    respuesta = input("¿Deseas exportar el reporte a PDF? [yes/y para confirmar]: ").strip().lower()
    return respuesta in ("yes", "y", "si", "sí", "s")
...
def run_app():
    ...
    if _preguntar_exportar_pdf():
        try:
            from core.pdf_exporter import exportar_reporte_pdf
            ruta = exportar_reporte_pdf(inventario, NOMBRE_TIENDA, TASAS_IMPUESTOS, calcular_precio_final)
            print(f"[PDF] Reporte exportado exitosamente: {ruta}")
        except ImportError:
            print("[ERR] No se encontró 'reportlab'. Activa el entorno virtual (.venv) e instala las dependencias:")
            print("      .venv\\Scripts\\activate  →  pip install -r requirements.txt")
    else:
        print("[INFO] Exportación a PDF cancelada.")
